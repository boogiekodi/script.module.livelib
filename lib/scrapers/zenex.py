...
def run(id,referer):
	type="rtmp"
	r_pageurl="http://www.zenexplayer.com/embedplayer/%s/1/680/470"%id
	...
	s=flashvars["s"][0]
	pk=flashvars["pk"][0]
	r_swfurl="http://www.zenexplayer.com%s"%re.findall('SWFObject\("(.*?)"',page)[0]
	values={}
	values["live"]="true"
	r_server='http://www.zenexpublisher.com:1935/loadbalancer?%s'%vidid
	r_server=bt.get_page(r_server,encoding).split("=")[1]
	r_tcurl="rtmp://%s/zenex"%r_server
